# Urvi Shah - 2D Graphics Portfolio

Welcome to my professional portfolio website! This is a modern, responsive portfolio showcasing my work as a Manager in 2D Graphics at INVNT.

## 📋 Overview

This portfolio features:
- **Hero Section**: Eye-catching introduction with call-to-action buttons
- **About Section**: Professional background and key skills
- **Projects Section**: Showcase of featured graphics projects
- **Contact Section**: Multiple ways to get in touch
- **Responsive Design**: Works seamlessly on desktop, tablet, and mobile

## 🎨 Live Demo

View the live portfolio at: [Add your GitHub Pages URL here]

## 🚀 Features

- Fully responsive design
- Smooth navigation with active link indicators
- Dynamic project loading from JavaScript
- Modern gradient designs and smooth animations
- Professional color scheme
- Integration with Behance portfolio link

## 📝 Customization Guide

### 1. Update Your Information

Edit the following in `index.html`:
- Replace contact email: `your.email@example.com`
- Update LinkedIn URL in the contact section
- Modify the hero section text to match your branding

### 2. Update Your Projects

Edit `script.js` and update the `projects` array with your actual projects:

```javascript
const projects = [
    {
        title: "Your Project Title",
        description: "Description of what you created and its impact",
        tags: ["Tag1", "Tag2", "Tag3"],
        link: "https://www.behance.net/yourprojects"
    },
    // Add more projects...
];
```

### 3. Customize Colors

Edit `styles.css` and modify the CSS variables in the `:root` selector:

```css
:root {
    --primary-color: #6366f1;
    --secondary-color: #ec4899;
    --dark-bg: #0f172a;
    --light-bg: #f8fafc;
    --text-primary: #1e293b;
    --text-secondary: #64748b;
    --border-color: #e2e8f0;
}
```

## 📂 File Structure

```
Urvi-Shah-Portfolio/
├── index.html          # Main HTML file
├── styles.css          # All styling
├── script.js           # Dynamic functionality and projects
└── README.md           # This file
```

## 🔧 Technologies Used

- HTML5
- CSS3 (Flexbox, Grid, Gradients)
- Vanilla JavaScript
- Responsive Design

## 📱 Browser Compatibility

- Chrome (Latest)
- Firefox (Latest)
- Safari (Latest)
- Edge (Latest)
- Mobile browsers (iOS Safari, Chrome Mobile)

## 🎯 Next Steps

1. Update project information with your actual work
2. Replace placeholder email with your contact email
3. Add more projects as you complete them
4. Consider deploying to GitHub Pages for free hosting
5. Add project images/thumbnails to replace the placeholder gradients

## 📧 Contact

- **Behance**: https://www.behance.net/urvishah2
- **Email**: your.email@example.com
- **LinkedIn**: https://linkedin.com/in/urvishah

## 📄 License

This portfolio is personal and proprietary. All rights reserved.

---

**Happy Coding! 🎨✨**